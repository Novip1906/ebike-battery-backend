...
deploy/k8s/                                манифесты стенда velo.14dev.ru
```

## Данные

14 режимов работы мотора: 12 опубликованных, один черновик (`Limit`, создан велосипедистом 1), один удалённый (`Off`).
...
Манифесты в `deploy/k8s`: пространство имён `ebike`, Minio и PostgreSQL с постоянными томами, Adminer (доступ через `kubectl port-forward svc/adminer 8081:80`), приложение и два Ingress на одном домене — второй отдаёт содержимое бакета по пути `/media`. Сертификат Let's Encrypt выпускается cert-manager.
